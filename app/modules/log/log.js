...
module.exports = function(spider) {
	...
	spider.agent.addListener('spiderEnd', function() {
		setTimeout( function() {
			msg('===========[' + spider.target + ']抓取完成,耗时:'+ ((new Date()) - _spiderStartTime)+'毫秒!==========');
	...
	spider.agent.addListener('addUrl', function(addUrl,unvisited,target) {
		...
		//require('../dao/log.dao.js')().save(target+'.unvisited',data);
	});
	//保存成功
	...
	spider.agent.addListener('useful', function(target,url) {
		...
		//require('../dao/log.dao.js')().save(target + '.useful',data);
	});
	spider.agent.addListener('unuseful', function(url) {
		var data  = {
			url:url
		};
		//require('../dao/log.dao.js')().save('unuseful',data);
	});
	spider.agent.addListener('visited', function(url,target) {
		var data  = {
			url:url
		};
		//require('../dao/log.dao.js')().save(target+'.visited',data);
	});
